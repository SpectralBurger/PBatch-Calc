...
chrome.runtime.onMessage.addListener(async (msg, sender, sendResponse) => {
    if (msg.action === "clickCalculateButton") {
        ...
        const spdField = document.querySelector('input[name="inPSpeedMph"]')
        const data = msg.data;
        ...
        const spdUpper = Number(data.spdUpper);
        const spdLower = Number(data.spdLower);
        const spdInterval = Number(data.spdInterval);
        ...
        if (btn) {
            for (let i1 = diamLower; i1 <= diamUpper; i1+= diamInterval) {
                for (let i2 = pitchLower; i2 <= pitchUpper; i2+= pitchInterval) {
                    for (let i3 = spdLower; i3 <= spdUpper; i3+= spdInterval) {
                        ...
                        LvlSpd = Number(document.querySelector("#outMLevelSpeed")?.innerText || 0);
